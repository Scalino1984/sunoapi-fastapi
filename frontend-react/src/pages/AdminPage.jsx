  ...
  async function saveAssistantSettings(patch = {}) {
    ...
    try {
      ...
      await api.admin.saveAiSettings({
        ...
        srt_alignment_engine: next.srt_alignment_engine === 'heuristic' ? 'heuristic' : 'forced_alignment',
            ...
            <article className="panel stack assistant-related-panel">
              ...
              <details className="assistant-related-details" open>
                ...
                <div className="form-grid compact-grid">
                  ...
                  <label>{t('admin.srt.alignmentEngine', 'Alignment-Engine')}<select value={settings.srt_alignment_engine === 'heuristic' ? 'heuristic' : 'forced_alignment'} onChange={(event) => setSettings({ ...settings, srt_alignment_engine: event.target.value })}><option value="forced_alignment">{t('admin.srt.engineForced', 'Forced Alignment (MMS/CTC, benötigt torch/torchaudio)')}</option><option value="heuristic">{t('admin.srt.engineHeuristic', 'Heuristik (ASR-Anker + Interpolation)')}</option></select></label>
