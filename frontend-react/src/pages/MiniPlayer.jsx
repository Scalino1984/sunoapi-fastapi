...
function fallbackAssetDuration(asset) {
  const assetDuration = positiveFiniteNumber(asset?.duration_seconds);
  const waveform = parseMaybeJson(asset?.waveform_json);
  const waveformDuration = positiveFiniteNumber(waveform?.duration_seconds);
  return waveformDuration || assetDuration || 0;
}

function resolvePlaybackDuration(nativeDuration, asset) {
  const nativeValue = positiveFiniteNumber(nativeDuration);
  return nativeValue || fallbackAssetDuration(asset);
}
...
export function MiniPlayer({ queue, currentIndex, loop, sidebarMode = 'open', mobileNavOpen = false, playerCommand = 0, onPlaybackStateChange, onLoopChange, onIndexChange, onOpenDetails, onPrepareMusic, onFavoriteChange, onClose }) {
  ...
  useEffect(() => {
    ...
    setIsPlaying(false);
    setCurrentTime(0);
    setDuration(fallbackAssetDuration(current));
    setMenuOpen(false);
    setExpanded(false);
  ...
  useEffect(() => {
    onPlaybackStateChange?.({
      ...
      isPlaying,
      currentTime,
      duration: duration || fallbackAssetDuration(current),
    });
  }, [current?.id, isPlaying, currentTime, duration, current?.duration_seconds, current?.waveform_json, onPlaybackStateChange]);
  ...
  const displayedSrtKey = displayedSrtSegment
    ? `${displayedSrtSegment.start || 0}-${displayedSrtSegment.end || 0}-${displayedSrtText}`
    : 'empty';
  ...
  }, [current?.id, hasSrt, displayedSrtText, displayedSrtSegment?.start, displayedSrtSegment?.end, currentTime, isPlaying, duration, current?.duration_seconds, current?.waveform_json]);
  ...
  function seekRelative(deltaSeconds) {
    if (!audioRef.current) return;
    const detectedDuration = resolvePlaybackDuration(audioRef.current.duration, current) || duration || 0;
  ...
  function seekFromEvent(event) {
    ...
    const seekDuration = resolvePlaybackDuration(audioRef.current.duration, current) || duration || 0;
    if (!(seekDuration > 0)) return;
    const rect = progressRef.current.getBoundingClientRect();
    const ratio = Math.min(1, Math.max(0, (event.clientX - rect.left) / rect.width));
    const target = Math.max(0, Math.min(seekDuration, ratio * seekDuration));
    audioRef.current.currentTime = target;
    setCurrentTime(target);
  }
        ...
        <div className={`mini-player-visual ${playerView === 'srt' && hasSrt ? 'is-srt' : 'is-waveform'}`}>
          {playerView === 'srt' && hasSrt ? (
            <div className="mini-player-srt-live" style={{ '--srt-live-font-size': displayedSrtFontSize }}>
              ...
              <strong key={displayedSrtKey} title={displayedSrtText}>{displayedSrtText || '\u00a0'}</strong>
