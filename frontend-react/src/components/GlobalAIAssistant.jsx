...
const starterHints = [
  ...
  'Mach meinen Song Suno-ready.',
  'Hilf mir beim Songtext.',
  'Öffne den passenden Workflow.'
];

function shortCanvas(text) {
  const value = String(text || '');
  if (value.length <= 900) return value;
  return `${value.slice(0, 900)}\n\n… gekürzt (${value.length} Zeichen insgesamt)`;
}
...
function describeDawPlan(plan) {
  const operations = Array.isArray(plan?.operations) ? plan.operations : [];
  if (!operations.length) return 'Kein Bearbeitungsplan erkannt.';
  return operations.map((op) => {
    ...
    if (op.type === 'gain') return `Lautstärke ${op.gain_db || 0} dB`;
...
function normalizeAssistantText(value) {
  if (value === null || value === undefined) return 'Ich habe die Antwort vorbereitet.';
  if (typeof value === 'string') {
    const trimmed = value.trim();
    if (!trimmed) return 'Ich habe die Antwort vorbereitet.';
    try {
  ...
  if (typeof value === 'object') {
    ...
    if (value.canvas_text || value.proposed_canvas) return 'Ich habe eine Canvas-Vorschau direkt im Songtext-Studio vorbereitet.';
    return 'Ich habe die Antwort vorbereitet.';
  }
  return String(value || '').trim() || 'Ich habe die Antwort vorbereitet.';
}
...
export function GlobalAIAssistant({ notify }) {
  ...
  async function refreshRuntimeInfo() {
    try {
      ...
      setRuntimeInfo(null);
      setRuntimeError(err.message || 'KI-Konfiguration nicht lesbar');
    }
  }
  ...
  function formatRuntimeLabel(info) {
    if (!info?.provider && !info?.model) return 'KI-Modell wird geladen …';
    const provider = info?.provider || 'Provider offen';
    const model = info?.model || 'Modell offen';
    return `${provider} · ${model}`;
  }

  function formatRuntimeDetail(info) {
    if (!info) return runtimeError || 'Admin-Konfiguration wird geprüft.';
    const chunks = [];
    if (info.profile_name) chunks.push(`Profil: ${info.profile_name}`);
    else chunks.push('Profil: Admin-Defaults');
    if (info.instruction_files_count !== undefined) chunks.push(`${info.instruction_files_count} Prompt-Baustein(e)`);
    if (info.vocal_tags_count !== undefined) chunks.push(`${info.vocal_tags_count} Vocal Tag(s)`);
    return chunks.join(' · ');
  }
  ...
  async function send(finalMessage = message, actionId = null, options = {}) {
    ...
    const requestContext = applyToCanvas
      ...
      : {
          ...
          current_canvas: shortCanvas(context?.current_canvas || '')
        };
    ...
    try {
      if (looksLikeSrtCommand(clean)) {
        ...
        const actions = [{ id: 'srt_focus_editor', label: 'SRT-Editor öffnen' }];
        if (addSegment) actions.unshift({ id: 'srt_add_segment', label: 'Segment jetzt hinzufügen', payload: { start: context?.current_audio_time || null } });
        if (shiftSegments) actions.unshift({ id: 'srt_shift_from_here', label: `Ab hier um ${seconds}s verschieben`, payload: { seconds, delta: seconds, include_current: true } });
        if (extendSegment) actions.unshift({ id: 'srt_extend_selected', label: `Segment um ${seconds}s verlängern`, payload: { seconds, delta: seconds, ripple: true } });
        setChat((items) => [...items, {
          id: entryId + 1,
          role: 'assistant',
          text: shiftSegments || extendSegment
            ? `SRT-Befehl erkannt. Ich kann den Editor öffnen und die Aktion mit ${seconds}s vorbereiten. Bitte kontrolliere die Vorschau vor dem Speichern.`
            : addSegment
              ? 'SRT-Befehl erkannt. Ich kann im Songdetail den Segment-Editor öffnen und an der aktuellen Wiedergabezeit ein neues Segment vorbereiten.'
              : 'SRT-Befehl erkannt. Ich kann den Live-Untertitel-Container und den Segment-Editor im aktuellen Library-Song öffnen.',
          actions
        }]);
        setMessage('');
        return;
      ...
      if (looksLikeAudioCommand(clean)) {
        ...
        if (dawResult?.is_audio_command) {
          const text = dawResult.plan
            ? `${dawResult.message || 'Audio-Befehl erkannt.'}

Geplanter DAW-Eingriff: ${describeDawPlan(dawResult.plan)}

Original bleibt unverändert. Beim Rendern wird eine neue Library-Version erstellt.`
            : (dawResult.message || 'Audio-Befehl erkannt, aber es fehlt noch eine eindeutige Auswahl.');
          setChat((items) => [...items, {
            id: entryId + 1,
            role: 'assistant',
            text,
            actions: dawResult.plan ? [
              { id: 'daw_execute_plan', label: 'Direkt rendern', payload: dawResult.plan },
              { id: 'navigate_daw', label: 'In Mini-DAW öffnen', payload: { audio_asset_id: dawResult.asset?.id || dawResult.plan?.source_audio_id } }
            ] : [{ id: 'navigate_daw', label: 'Mini-DAW öffnen' }],
          }]);
          setMessage('');
          return;
        }
      ...
      let replyText = normalizeAssistantText(response.reply);
      const proposed = response.proposed_canvas || null;
      if (proposed) {
        const previewPayload = {
          text: proposed,
          summary: response.change_summary || 'KI-Vorschau vorbereitet',
        ...
        replyText = replyText && !replyText.includes('canvas_text') ? replyText : 'Ich habe eine Canvas-Vorschau direkt im Songtext-Studio vorbereitet.';
      }
      const assistantEntry = {
        ...
        changeSummary: proposed ? (response.change_summary || 'Vorschau liegt im Canvas bereit.') : response.change_summary
      };
      setChat((items) => [...items, assistantEntry]);
      setMessage('');
    } catch (err) {
      const text = err.message || 'Der KI-Assistent konnte nicht antworten.';
  ...
  async function runAction(action) {
    ...
    if (String(actionId) === 'daw_execute_plan') {
      try {
        ...
        setChat((items) => [...items, { id: Date.now(), role: 'assistant', text: `Erledigt: Neue DAW-Version „${result.version_label || result.display_title || result.id}“ wurde gespeichert.` }]);
      } catch (err) {
        const text = err.message || 'DAW-Render fehlgeschlagen.';
        notify?.(text, 'error');
    ...
    if (String(actionId).startsWith('lyrics_') && actionId !== 'lyrics_save' && actionId !== 'lyrics_apply_preview' && actionId !== 'lyrics_discard_preview') {
      await send(action?.label || 'Songtext bearbeiten', actionId, { applyToCanvas: true });
      return;
    }
    ...
    if (ok) {
      ...
      setChat((items) => [...items, { id: Date.now(), role: 'assistant', text: `Erledigt: ${action?.label || actionId}` }]);
    } else {
      notify?.('Diese Aktion ist hier noch nicht direkt ausführbar.', 'error');
    }
  }

  if (!open) {
    return (
      <button type="button" className="global-ai-toggle" onClick={() => setOpen(true)} title="KI-Assistent öffnen">
  ...
  return (
    <aside className={`global-ai-panel ${minimized ? 'is-minimized' : ''}`}>
      <div className="global-ai-header">
        <div>
          <strong><Bot size={18} /> KI-Assistent</strong>
          <span>{pageLabel}</span>
        ...
        <div className="global-ai-header-actions">
          <button type="button" onClick={() => { setChat([]); setPendingCanvas(null); setMessage(''); localStorage.removeItem('global-ai-chat'); notify?.('Chatfenster wurde geleert.', 'success'); }} title="Chat leeren"><Trash2 size={16} /></button>
          <button type="button" onClick={() => setMinimized(!minimized)} title={minimized ? 'Ausklappen' : 'Einklappen'}>{minimized ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>
          <button type="button" onClick={() => setOpen(false)} title="Schließen"><X size={16} /></button>
        </div>
      </div>
        ...
        <>
          <section className={`global-ai-top-container ${quickPanelCollapsed ? 'is-collapsed' : ''}`}>
            <button
              ...
              title={quickPanelCollapsed ? 'Kontext und Schnellaktionen anzeigen' : 'Kontext und Schnellaktionen einklappen'}
            >
              <span>Kontext & Schnellaktionen</span>
              <small>{formatRuntimeLabel(runtimeInfo)} · {pageActions.length || 0} Aktion(en)</small>
            ...
            {!quickPanelCollapsed && (
              <div className="global-ai-top-content">
                <div className="global-ai-context">
                  <span>Ich erkenne automatisch, wo du gerade bist, und biete passende Schritte an.</span>
                  <div className="global-ai-runtime-box">
                    <strong>Aktive KI</strong>
          ...
          <div className="global-ai-log" ref={logRef}>
            {!chat.length && (
              <div className="global-ai-empty">
                <strong>Neue Session</strong>
                <p>Das Nachrichtenfeld ist leer. Du kannst frei schreiben oder einen passenden Befehl für den aktuellen Schritt nutzen.</p>
            ...
            {chat.map((item) => (
              <div key={item.id} className={`global-ai-message ${item.role}`}>
                <strong>{item.role.startsWith('user') ? 'Du' : 'Assistent'}</strong>
            ...
            {pendingCanvas && (
              <div className="global-ai-preview compact-preview">
                <strong>Canvas-Vorschau vorbereitet</strong>
                <p className="muted">{pendingCanvas.summary}</p>
                <p>Der Songtext wird nicht im Chat angezeigt. Prüfe ihn direkt im Canvas des Songtext-Studios.</p>
                <div className="global-ai-preview-actions">
                  <button className="primary" type="button" onClick={() => runAction({ id: 'lyrics_apply_preview', label: 'Canvas übernehmen' })}><Check size={15} /> Übernehmen</button>
                  <button type="button" onClick={() => runAction({ id: 'lyrics_discard_preview', label: 'Verwerfen' })}><Trash2 size={15} /> Verwerfen</button>
                </div>
              </div>
            )}
          </div>
          ...
          <form className="global-ai-input" onSubmit={(event) => { event.preventDefault(); send(); }}>
            <textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Frag nach Lyrics, Styles, SRT-Segmenten, Projekten oder einfachen Audio-Bearbeitungen …" rows={3} onKeyDown={(event) => { if (event.ctrlKey && event.key === 'Enter') send(); }} />
            <button className="primary" type="submit" disabled={busy}>{busy ? <Loader2 className="spin-icon" size={16} /> : <Send size={16} />} Senden</button>
          </form>
        </>
      )}
    </aside>
