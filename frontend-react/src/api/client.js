...
export const api = {
  ...
  admin: {
    ...
    instructionFiles: (includeContent = false) => apiFetch(`/api/admin/instruction-files?include_content=${includeContent ? 'true' : 'false'}`),
    createInstructionFile: (payload) => apiFetch('/api/admin/instruction-files', { method: 'POST', body: JSON.stringify(payload) }),
    updateInstructionFile: (id, payload) => apiFetch(`/api/admin/instruction-files/${id}`, { method: 'PUT', body: JSON.stringify(payload) }),
