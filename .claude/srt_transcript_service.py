...
def _effective_find_occurrences(
    ...
    *,
    ...
    window_sizes = sorted({candidate_size for candidate_size in (size - 1, size, size + 1) if candidate_size >= 2})
    if not window_sizes:
        window_sizes = [size]
...
def _script_candidate_line_score(candidate_tokens: list[str], line: LyricLine) -> float:
    ...
    for variant in _effective_line_token_variants(line_tokens) or [line_tokens]:
        ...
        overlap_count = len(candidate_set & variant_set)
        overlap = (2 * overlap_count) / max(1, len(candidate_set) + len(variant_set))
        prefix_bonus = 0.06 if candidate_tokens[:2] == variant[:2] else 0.0
        best = max(best, min(1.0, max(seq, overlap) + prefix_bonus))
    return round(best, 3)
...
def _script_match_asr_candidates_to_lyrics(
    ...
    *,
    ...
    matches: list[_AsrLyricsLineMatch] = []
    search_start = 0
    for candidate in candidates:
        best_index: int | None = None
        best_score = 0.0
        start_index = max(0, search_start)
        for line_index in range(start_index, len(lines)):
            score = _script_candidate_line_score(candidate.tokens, lines[line_index])
            if score > best_score:
                best_score = score
                best_index = line_index
        if best_index is None or best_score < min_score:
            continue
        matches.append(_AsrLyricsLineMatch(
        ...
        search_start = max(search_start, best_index + 1)
    return matches
...
def _script_apply_asr_line_review(lines: list[LyricLine], hyp: list[HypWord]) -> list[str]:
    ...
    report = [
        f"INFO: ASR-Zeilenabgleich: {len(candidates)} Kandidaten, {len(matches)} sichere Lyrics-Treffer."
    ]
    changed = 0
