...
# Order matters: the most specific tags must be detected before generic tags.
# Suno lyrics often use flexible spellings such as ``[Rap-Verse-1]`` or
# ``[Post Chorus]``. The patterns therefore match the semantic main words
# instead of one fixed literal spelling.  These labels are used for waveform
# and SRT section segmentation only; descriptor-only tags are intentionally
# ignored.
SECTION_PATTERNS: tuple[tuple[str, str, str], ...] = (
    (r"\binstrumental\s*[- ]?outro\b", "Instrumental Outro", "instrumental_outro"),
    (r"\binstrumental\s*[- ]?intro\b", "Instrumental Intro", "instrumental_intro"),
    (r"\bintro\b|\bintrro\b|\binttro\b", "Intro", "intro"),
    (r"\binstrumental\s*[- ]?break\b", "Instrumental Break", "instrumental_break"),
    (r"\bpre\s*[- ]?verse\b", "Pre-Verse", "pre_verse"),
    (r"\brap\s*[- ]?verse\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Rap Verse", "rap_verse"),
    (r"\bspoken\s*[- ]?verse\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Spoken Verse", "spoken_verse"),
    (r"\bverse\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Verse", "verse"),
    (r"\bvers\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Verse", "verse"),
    (r"\bstrophe\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Verse", "verse"),
    (r"\bpart\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Verse", "verse"),
    (r"\bpre\s*[- ]?hook\b", "Pre-Hook", "pre_hook"),
    (r"\bpost\s*[- ]?hook\b", "Post-Hook", "post_hook"),
    (r"\bpre\s*[- ]?chorus\b", "Pre-Chorus", "pre_chorus"),
    (r"\bpost\s*[- ]?chorus\b", "Post-Chorus", "post_chorus"),
    (r"\b(?:final|last)\s*[- ]?(?:chorus|chours|hook|refrain)\b", "Final Chorus", "chorus"),
    (r"\bchorus\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Chorus", "chorus"),
    (r"\bchours\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Chorus", "chorus"),
    (r"\bhook\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Hook", "hook"),
    (r"\brefrain\s*[- ]?(?P<number>\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten)?\b", "Refrain", "refrain"),
    (r"\bbridge\b|\bbrigde\b", "Bridge", "bridge"),
    (r"\bbreak\s*[- ]?down\b|\bbreakdown\b", "Breakdown", "breakdown"),
    (r"\bbreak\b", "Break", "break"),
    (r"\binterlude\b", "Interlude", "interlude"),
    (r"\binstrumental\b", "Instrumental", "instrumental"),
    (r"\bdrop\b", "Drop", "drop"),
    (r"\bbuild\s*[- ]?up\b", "Build-Up", "build_up"),
    (r"\bbuild\b", "Build", "build"),
    (r"\bclimax\b", "Climax", "climax"),
    (r"\bguitar\s*[- ]?solo\b", "Guitar Solo", "solo"),
    (r"\bpiano\s*[- ]?solo\b", "Piano Solo", "solo"),
    (r"\bdrum\s*[- ]?solo\b", "Drum Solo", "solo"),
    (r"\bbass\s*[- ]?solo\b", "Bass Solo", "solo"),
    (r"\bsolo\b", "Solo", "solo"),
    (r"\bspoken\s*[- ]?word\b", "Spoken Word", "spoken_word"),
    (r"\bad\s*[- ]?libs?\b", "Ad-Libs", "adlibs"),
    (r"\bcall\s*(?:and|&)\s*response\b", "Call and Response", "call_response"),
    (r"\bchoir\b", "Choir", "choir"),
    (r"\bbacking\s*[- ]?vocals?\b", "Backing Vocals", "backing_vocals"),
    (r"\bbackground\s*[- ]?vocals?\b", "Background Vocals", "background_vocals"),
    (r"\boutro\b|\bouttro\b", "Outro", "outro"),
    (r"\bend\b", "End", "end"),
    (r"\bfade\s*[- ]?in\b", "Fade In", "fade_in"),
    (r"\bfade\s*[- ]?out\b", "Fade Out", "fade_out"),
)
...
def build_waveform_payload(asset: AudioAsset, path: Path, points: int = 180) -> dict[str, Any]:
    measured_duration = read_audio_duration_seconds(path)
    duration = measured_duration or asset.duration_seconds
...
def get_or_create_waveform(asset: AudioAsset, path: Path, db: Session, points: int = 180, rebuild: bool = False) -> dict[str, Any]:
    ...
    if not rebuild and isinstance(existing, dict) and existing.get("peaks") and int(existing.get("points") or 0) >= min(points, 60):
        ...
        if measured_duration and abs(float(payload.get("duration_seconds") or 0) - float(measured_duration)) > 0.05:
            payload = dict(payload)
            payload["duration_seconds"] = measured_duration
            changed = True
        if changed:
            ...
            db.commit()
            db.refresh(asset)
        return sanitize_waveform_payload_for_asset(asset, payload, duration_seconds=measured_duration) or payload
