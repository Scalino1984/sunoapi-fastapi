...
def _script_align_lines(lines: list[LyricLine], hyp: list[HypWord], warn_factor: float = 0.6) -> list[str]:
    ...
    report.extend(_script_resolve_timeline(lines, warn_factor, hyp_first_start=hyp_first_start))
...
def _script_tail_spread_section_transition(
    ...
    dem Intro-Ende falsch: Dann würden Verse-Zeilen viel zu früh erscheinen. Für
    diesen Spezialfall werden die nicht gematchten Zeilen in ein lesbares Fenster
    direkt vor den nächsten stabilen Anker gelegt.
    """
    ...
    gap_lines = max(1, anchor_b - anchor_a - 1)
    # Root-Schutz gegen spaete Verse-Starts: Bei ASR-Luecken darf ein neuer
    # Abschnitt nicht nur in die letzten Sekunden vor dem naechsten Anchor
    # gequetscht werden. Genau das erzeugt den Fehler „Verse beginnt real bei
    # 25s, SRT startet aber erst beim Anchor um 30s“. Wir reservieren deshalb
    # ein Mindestfenster pro Lyrics-Zeile und einen prozentualen Anteil der
    # offenen Luecke. Vocal-Stems verbessern die Anker; diese Guard verhindert
    # zusaetzlich systematische Spaetstarts fuer alle Songs.
    min_section_window = max(required, gap_lines * 1.35, 4.5)
    tail_lead = min(max(5.0, window * 0.28), 11.0)
    reserved = max(min_section_window, tail_lead)
    if window < max(9.0, min_section_window + 3.0):
        return None, None

    tail_start = max(window_start, window_end - reserved)
    ...
    return tail_start, (
        ...
        f"({window_start:.1f}s-{window_end:.1f}s -> {tail_start:.1f}s-{window_end:.1f}s, reserviert {reserved:.1f}s)."
    )
...
def _script_resolve_timeline(
    ...
    for anchor_a, anchor_b in zip(anchors, anchors[1:]):
        ...
        tail_start, tail_report = _script_tail_spread_section_transition(lines, anchor_a, anchor_b, window_start, window_end)
