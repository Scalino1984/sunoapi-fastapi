...
function libraryExportPath(kind, format = 'csv', mode = 'extended') {
  return `/api/library/export/${encodeURIComponent(kind)}?format=${encodeURIComponent(format)}&mode=${encodeURIComponent(mode)}`;
}

function importLibraryFile(kind, file, format = 'auto') {
  const formData = new FormData();
  formData.append('file', file);
  return apiFetch(`/api/library/import/${encodeURIComponent(kind)}?format=${encodeURIComponent(format)}`, {
    method: 'POST',
    body: formData,
    timeoutMs: 60000
  });
...
export const api = {
  ...
  archive: {
    ...
    // Aktiver SRT-Workflow: Library/MiniPlayer nutzen die AudioAsset-SRT-Routen.
    // Nicht durch die alte, derzeit nicht eingebundene SrtTimelineEditor/api.srt.validate-Kette ersetzen.
  ...
  library: {
    ...
    // Aktive Seiten rufen diese Import-/Export-Wrapper direkt auf.
    // Backend-Routen existieren in app/routers/library.py; fehlende Wrapper würden sonst erst zur Laufzeit im UI brechen.
    exportLyrics: (format = 'csv', mode = 'extended') => apiFetch(libraryExportPath('lyrics', format, mode)),
    importLyrics: (file, format = 'auto') => importLibraryFile('lyrics', file, format),
    exportPlaylists: (format = 'csv', mode = 'extended') => apiFetch(libraryExportPath('playlists', format, mode)),
    importPlaylists: (file, format = 'auto') => importLibraryFile('playlists', file, format),
    exportStyles: (format = 'csv', mode = 'extended') => apiFetch(libraryExportPath('styles', format, mode)),
    importStyles: (file, format = 'auto') => importLibraryFile('styles', file, format),
    exportVocalTags: (format = 'csv', mode = 'extended') => apiFetch(libraryExportPath('vocal-tags', format, mode)),
    importVocalTags: (file, format = 'auto') => importLibraryFile('vocal-tags', file, format),
  ...
  system: {
    ...
    // SystemPage nutzt diese dedizierte Route für Dry-Run und bestätigtes Caching externer Cover.
    // Nicht mit api.archive.cacheMissingCovers verwechseln: andere Semantik, kein Dry-Run, anderes Resultatformat.
