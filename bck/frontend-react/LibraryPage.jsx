...
export function LibraryPage({ assets, loadError = '', voices = [], playlists = [], onReload, onPlay, notify, onUseLyric, onReusePrompt, openAssetId, openAssetRequestKey = 0, onOpenAssetHandled, resetSignal = 0, onOpenDaw, playbackState = {}, onToggleCurrentPlayback, onDetailTitleChange, routeDetailSlug = '', searchQuery = '', onTrashChanged }) {
  ...
  const [playerSrtLineState, setPlayerSrtLineState] = useState(null);
  ...
  useEffect(() => {
    function handlePlayerSrtLine(event) {
      ...
      playerSrtLineRef.current = nextLine;
      setPlayerSrtLineState((current) => {
        if (
          String(current?.assetId || '') === String(nextLine.assetId || '')
          && String(current?.text || '') === String(nextLine.text || '')
          && Number(current?.start || 0) === Number(nextLine.start || 0)
          && Number(current?.end || 0) === Number(nextLine.end || 0)
          && Math.abs(Number(current?.currentTime || 0) - Number(nextLine.currentTime || 0)) < 0.25
          && Boolean(current?.hasSrt) === Boolean(nextLine.hasSrt)
          && Boolean(current?.isPlaying) === Boolean(nextLine.isPlaying)
        ) {
          return current;
        }
        return nextLine;
  ...
  function liveSrtLineForAsset(asset) {
    ...
    const playerSrtLine = playerSrtLineRef.current || playerSrtLineState;
    const eventLine = String(playerSrtLine?.assetId || '') === assetId ? playerSrtLine : null;
    const eventTime = Number(eventLine?.currentTime ?? playbackState?.currentTime ?? 0);
    const fallbackTime = Number.isFinite(eventTime) ? eventTime : Number(playbackState?.currentTime || 0);
    const eventStart = Number(eventLine?.start || 0);
    const eventEnd = Number(eventLine?.end || 0);
    const eventText = String(eventLine?.text || '').trim();
    const eventLineMatchesCurrentTime = Boolean(
      eventLine
      && eventLine.hasSrt !== false
      && eventText
      && eventEnd > eventStart
      && fallbackTime >= eventStart - 0.05
      && fallbackTime < eventEnd + 0.05
    );

    if (eventLineMatchesCurrentTime) {
      return {
        assetId: asset.id,
        text: eventText,
        start: eventStart,
    ...
    const visible = findActiveSrtSegment(segments, fallbackTime);
  ...
  function SrtCard({ asset }) {
    ...
    const visibleKey = visibleSegment ? `${visibleSegment.start || 0}-${visibleSegment.end || 0}-${visibleText}` : 'empty';
    return (
      <div className="meta-card wide srt-card">
        ...
        {hasSrt && (
          <div className={`srt-live-container ${isCurrentAsset(asset) ? 'is-live' : ''}`} style={srtLiveColorStyle}>
            ...
            <strong key={visibleKey}>{visibleText || '\u00a0'}</strong>
